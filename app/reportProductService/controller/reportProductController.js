...
const PRODUCTREPORT = require("../model/reportProductModel")
...
_report.reportUserandPost= async(req,res)=>{
    try {
        ...
        let result= await PRODUCTREPORT.create(data)
        res.status(200).send({
            success:true,
...
_report.getListing= async(req, res, next)=>{
    try {
        ...
        //     next()
        // }
        let count= await PRODUCTREPORT.find( { reportedTo: { $exists: true } }).countDocuments()
        let result= await PRODUCTREPORT.find( { reportedTo: { $exists: true } } )
...
_report.getPostListing= async(req, res, next)=>{
    try {
        ...
        let count= await PRODUCTREPORT.find( { postReport: { $exists: true } } ).countDocuments()
        let result= await PRODUCTREPORT.find( { postReport: { $exists: true } } )
        .populate({path:'reportedBy', select :'-password'})
...
_report.getReportById= async(req, res)=>{
    try {
        let result= await PRODUCTREPORT.find({_id:req.params.id})
        if(!result){
            res.status(400).send({
                success:false,
...
_report.deleteReportById= async(req, res)=>{
    try {
        let result= await PRODUCTREPORT.findByIdAndRemove({_id:req.params.id})
        if(!result){
            res.status(400).send({
                success:false,
                message:responseMessage.RECORD_NOTFOUND('Report')
...
_report.editReportById= async(req, res)=>{
    try {
        let data=req.body;
        let result= await PRODUCTREPORT.findByIdAndUpdate({_id:req.params.id},data,{new:true})
        if(!result){
            res.status(400).send({
                success:false,
                message:responseMessage.RECORD_NOTFOUND('Report')
